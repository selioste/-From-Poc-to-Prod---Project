...
class TestBaseTextCategorizationDataset(unittest.TestCase):
    ...
    def test_get_index_to_label_map(self):
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base._get_label_list = MagicMock(return_value=["tomato", "pommel"])
        self.assertEqual(base.get_index_to_label_map(), {0: "tomato", 1: "pommel"})

    def test_index_to_label_and_label_to_index_are_identity(self):
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base.get_index_to_label_map = MagicMock(return_value=["tomato", "pommel"])
        self.assertEqual(base.get_label_to_index_map(), {"tomato": 0, "pommel": 1})

    def test_to_indexes(self):
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        base.get_label_to_index_map = MagicMock(return_value={"tomato": 0, "pommel": 1})
        self.assertEqual(base.to_indexes(["tomato", "pommel"]), [0, 1])
