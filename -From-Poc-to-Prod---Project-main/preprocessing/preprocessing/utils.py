...
class BaseTextCategorizationDataset:
    ...
    def get_label_to_index_map(self):
        ...
        """
        # TODO: CODE HERE
        return {self._get_label_list()[i]: i for i in range(len(self._get_label_list()))}
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                filtered_tags = df['tag_id'].value_counts() >= x
                index = filtered_tags[filtered_tags == True].index
