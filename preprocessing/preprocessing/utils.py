...
class BaseTextCategorizationDataset:
    ...
    def _get_num_train_samples(self):
        ...
        """
        # TODO: CODE HERE
        num_train_samples = self.batch_size * self.train_ratio
    ...
    def _get_num_test_samples(self):
        ...
        """
        # TODO: CODE HERE
        num_test_samples = self.batch_size * (1 - self.train_ratio)
    ...
    def _get_num_train_batches(self):
        ...
        """
        # TODO: CODE HERE
        # num_train_batches = (self.batch_size * self.train_ratio) // self.batch_size
        num_train_batches = self._get_num_train_samples() / self.batch_size
    ...
    def _get_num_test_batches(self):
        ...
        """
        # TODO: CODE HERE
        # num_test_batches = (self.batch_size * (1 - self.train_ratio)) // self.batch_size
        num_test_batches = self._get_num_test_samples() / self.batch_size
    ...
    def get_index_to_label_map(self):
        ...
        """
        # TODO: CODE HERE
        mapped_index_to_label = {}
        for idx, label in enumerate(self._get_label_list()):
            mapped_index_to_label[idx] = label
            return mapped_index_to_label
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def __init__(self, filename, batch_size,
        ...
        self._label_list = self._dataset
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        # TODO: CODE HERE
        load_dataset = pd.read_csv(filename, sep=',')

        # assert that columns are the ones expected
        # TODO: CODE HERE
        assert load_dataset.columns.to_list() == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                filter_tags = (df['tag_name'].value_counts() >= x)
                filter_tags_values = filter_tags[filter_tags.values]
                filter_tags = filter_tags_values.index.to_list()
                df = df[df['tag_name'].isin(filter_tags)]
                return df
        ...
        # TODO: CODE HERE
        preprocess_dataset = load_dataset.pipe(filter_tag_position(0))
        return preprocess_dataset.pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
